...
task pgdac_mo_nmf {
	...
	command {
		set -euo pipefail
		#Command goes here
		Rscript /home/pgdac/src/mo-nmf.r -t ${tar_file} -b ${default=false bnmf} -l ${default=2 kmin} -m ${default=6 kmax} -n ${default=100 nrun} -s ${default="random" seed} -c ${class_variable} -o ${other_variable} -d ${class_colors} -r ${default=false no_plot} -f ${default="0.1" sd_min} -u ${default=true z_score} -i ${default=false impute} -a ${default="geneSymbol" gene_column} -e ${default=true exclude_2} -z /home/pgdac/src/
	...
	runtime {
		docker : "broadcptac/pgdac_mo_nmf:12"
