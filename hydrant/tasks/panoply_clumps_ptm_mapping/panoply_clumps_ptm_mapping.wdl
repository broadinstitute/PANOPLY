...
task panoply_clumps_ptm_mapping {
	...
	command {
		...
		# Run Mapping Scripts 
		python /prot/proteomics/Projects/PGDAC/src/clumps_ptm_mapping.py ${'--phosphoproteome_gct ' + pSTY_gct} ${'--acetylome_gct ' + acK_gct} ${'--ubiquitylome_gct ' + ubK_gct} --PDB_DIR /pdbs --FASTA_ref_file ${FASTA_ref_file} ${'--accession_col ' + accession_col} ${'--variable_sites_col ' + variable_sites_col} ${'--variable_sites_sep ' + '"' + variable_sites_sep + '"'} ${'--gene_column ' + gene_column} --output_prefix ${output_prefix} --yaml ${yaml_file} --num_threads ${num_threads} \
