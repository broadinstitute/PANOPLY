...
for(ome in ome_types) {
  ...
  for(file in files_tmp) {
    ...
    html = includeHTML(file) %>% # read in HTML file
      gsub("return rect.width", "return cel.offsetWidth", .) %>% # modify HTML code for proper sizing inside .{tabset}
      gsub("return rect.height", "return cel.offsetHeight", .) # modify HTML code for proper sizing inside .{tabset}
    print(htmltools::tagList(tags$iframe(srcdoc=html, # insert widget into iframe
