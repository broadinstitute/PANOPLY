...
nmf_xlsx <- lapply(nmf_xlsx_sheets, function(x) {
  read_excel(xlsx_file, sheet=x) %>%  # read in each sheet
    mutate(Directionality = ifelse(!!as.name(glue("t.{sub('C','',x)}.over.rest"))>0,"up","down")) %>% # get sign of t-test
    dplyr::select(one_of(c('cluster', 'id_og', 'ome_type', # select relevant columns
                           'Directionality',
...
In total ```r nrow(nmf_xlsx_comb)``` features separating the clusters have been detected using the ```r feature_method``` method described above. The distribution of features across the different clusters are shown in **Figure `r fig_count`**. 
