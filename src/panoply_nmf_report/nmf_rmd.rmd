...
params:
  ...
  nmf_results: NULL
  nmf_parameters: NULL
  postprocess_parameters: NULL
  gct_expr_comb_file: NULL
  gct_expr_comb_nn_file: NULL
  rank_top: NULL
...
# read in data
load(params$nmf_results) # load res.rank list object (results of nmf() function)
# load in parameters
load(params$nmf_parameters) # load opt list object (panoply_nmf parameters)
opt_full = opt
load(params$postprocess_parameters) # load opt list object (panoply_nmf parameters)
opt_full = c(opt_full, opt) 

ranks = opt_full$kmin:opt_full$kmax # get range of clusters run
rank_top = params$rank_top # get 'best' cluster assignment

gct_expr_comb = parse_gctx(params$gct_expr_comb_file)
gct_expr_comb_nn = parse_gctx(params$gct_expr_comb_nn_file)
...
show_tab_feat = (dim(tab_feat)[1]>1) # determine if there's more than one ome-type, to toggle the table on/off

```

The data matrix subjected to NMF analysis contained `r nrow(gct_expr_comb@mat)` features measured across `r ncol(gct_expr_comb@mat)` samples. `r if (show_tab_feat) { paste0(" **Table ", tab_count, "** summarizes the number of features used in the clustering and their data type(s).")} else {paste0("All features used in this clustering analysis are of the same data-type.")}`

```{r tab_feat, echo=F, message=FALSE, warning=FALSE, eval=show_tab_feat}
...
```{r, include=TRUE, out.width='100%', out.height='100%', fig.align='left', fig.pos='H', fig.cap=paste0('**Figure ', fig_count,'**: Heatmap depicting the relative contributions of each sample (x-axis) to each cluster (y-axis). Samples are ordered by cluster, cluster core and cluster membership score in decreasing order (from left to right).'), echo=FALSE}
...
membership <- read.table(list.files(pattern = "*_clusterMembership.tsv"))
...
```{r, include=TRUE, out.width='100%', out.height='100%', fig.align='left', fig.pos='H', fig.cap=paste0('**Figure ', fig_count,'**: Heatmap depicting abundances of cluster-specific features defined as descibed above. Samples are ordered by cluster, cluster core and cluster membership score in decreasing order (from left to right).'), echo=FALSE}
...
nmf_xlsx <- lapply(nmf_xlsx_sheets, function(x) {
  read_excel(xlsx_file, sheet=x) %>%  # read in each sheet
    dplyr::select(one_of(c('id_og', 'ome_type', # select relevant columns
                           # 'Direction',
...
```{r feat_barplot, include=TRUE, include=TRUE, out.width='75%', fig.align='left', fig.pos='H', fig.cap=paste0('**Figure ', fig_count,'**: Barchart depicting the number of cluster-specific features (y-axis) per cluster (x-axis)'), echo=FALSE}
...
```{r display_feat_xlsx, fig.cap=paste0('**Table ', tab_count,'**: Data table containing all significant cluster-specific features.'), echo=F}
...
```{r, include=TRUE, fig.align='center', out.width='100%', out.height='100%', fig.cap=paste0('**Figure ', fig_count, '**: Consensus matrix derived from ', opt_full$nrun,' randomly initialized iterations.'), echo=FALSE}
knitr::include_graphics(list.files(pattern = "*_consensusMap.png"))#pdf"))
```
```{r echo=F}
## increment
fig_count <- fig_count + 1
