#!/bin/bash
while getopts ":c:o:r:a:s:l:p:x:y:" opt; do
    case $opt in
	...
	a) analysis_dir="$OPTARG";;
        ...
        p) ptmsea="$OPTARG";;
        x) ssgsea_assoc_tars="$OPTARG";;
        y) ssgsea_clust_tars="$OPTARG";;
...
{
  cd $src;
  mkdir -p $ssgsea_assoc;
  mkdir -p $ssgsea_clust;
  echo -e "$ssgsea_assoc_tars";
  echo -e "${ssgsea_assoc_tars[1]}";
  #IFS ";" read -ra assoc_array <<< "$ssgsea_assoc_tars";
  #IFS ";" read -ra clust_array <<< "$ssgsea_clust_tars";
  #for index in "${!assoc_array[@]}"
  #do
  #  cp ${assoc_array[index]} $ssgsea_assoc/ssgsea-assoc-$index.tar;
  #done
  #for index in "${!clust_array[@]}"
  #do
  #  cp ${clust_array[index]} $ssgsea_clust/ssgsea-clust-$index.tar;
  #done
}
...
dir_create
collect
tar -czvf pgdac_main_summary.tar.gz $dir/
tar -czvf pgdac_main_full.tar.gz $full/
