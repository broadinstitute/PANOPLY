...
## label      - character, name of folder in the tarball
## tmp.dir    - folder used to untar and write output
rmd_ssgsea <- function(tar.file, label='panoply-lscc-2-0-acetylome-full-results', label.rmd='rmd-ssgsea', type='acetylproteome', tmp.dir=tempdir(), res.dir='ssgsea_clust'){    
  ...
  ######################################
  ## idntifiy sub folders
  ## - each subfolder corresponds to one ssgsea run
  subfold <- dir(file.path(tmp.dir, label, res.dir ), full.names = T)
  idx <- sapply(subfold, dir.exists)
  subfold <- subfold[ idx ]
...
## ################################################
## run
rmd_ssgsea(tar.file=tar.file, label=label, tmp.dir=tmp.dir, type=type)
