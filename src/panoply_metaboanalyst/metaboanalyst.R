...
opt_cmd <- parse_args( OptionParser(option_list=option_list),
                       ...
                       args = c('--metabolome_gct',"opt/input/ODG-v2_2-metabolomics_log_norm.gct",
...
enrichementAnalysis <- function(gct, cov_of_interest, value_of_interest, label,
  ...
  mSet<-CreateMappingResultTable(mSet) # not clear what this does
  ...
  # Replace missing values with minimum concentration levels (required, even when unnecessary)
  ...
  mSet<-SetCurrentMsetLib(mSet, glue("{label}_smpdb_pathway"), 0); # might be able to change pathways here
...
for (value_of_interest in unique(gct@cdesc[[cov_of_interest]])) {
  enrichementAnalysis(gct, cov_of_interest, value_of_interest, label = glue("{opt$output_prefix}_{value_of_interest}"))
}
