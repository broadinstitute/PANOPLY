...
compound_map = qs::qread(file.path(opt$lib_dir, "compound_db.qs")) # mapping between ID types for metabolic compounds


#### Read in / Format Pathways ####
if (multiomic) {
  # format QS object from MetaboAnalyst Site into GMT-adjacent
  pathways_qs = qs::qread(file.path(opt$lib_dir,"hsa.qs"))
  ...
  pathways_gmt = read.GMT(file.path(opt$lib_dir, "smpdb_pathway.gmt"))
