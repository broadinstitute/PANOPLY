...
* ```input_gct```: (`.gct` file) normalized data table in the form of a GCT.
...
* ```tar_out```: An output tar file that contains the outlier analysis results in a directory called "blacksheep." Subdirectories with the results for "positive" and "negative" outlier analysis include:
