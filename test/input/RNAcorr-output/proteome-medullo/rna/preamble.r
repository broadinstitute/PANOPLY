...
rna.version <- 1
rna.data.file <- 'microarray-data.gct'
...
compute.cluster.type <- 'slurm'
cluster.queue <- 'compute'
scratch.fs <- sprintf ('%s/pid-%s/', '/oasis/scratch/comet/manidr/temp_project', Sys.getpid())
